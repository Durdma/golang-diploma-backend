...
func GenerateVerificationEmailAdmin(input AddEmailInput) (string, error) {
	h := generateInfo()
			...
			Actions: []hermes.Action{
				{
					...
					Button: hermes.Button{
						Text: "Подтвердить регистрацию",
						Link: "http://localhost:8080/api/v1/admins/verify/" + input.VerificationCode,
					},
				},
			},
			Outros: []string{
				"Ссылка для подтверждения перехода, если не работает кнопка:",
				"http://localhost:8080/api/v1/admins/verify/" + input.VerificationCode,
				"Если есть вопросы или вы столкнулись с проблемами пишите на данный адрес:",
				"durdma840@gmail.com",
			},
		},
